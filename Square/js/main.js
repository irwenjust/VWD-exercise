	...
	for(var i=0;i<squares.num;i++){
		if(squares.isLive[i]){
			if(squares.color[i]!=mySquare.color){
				if(mySquare.isLive){
					if(((mySquare.x+mySquare.len-1>squares.x[i]&&mySquare.x+mySquare.len-1<squares.x[i]+squares.w[i]
						&&mySquare.x+mySquare.len-30>squares.x[i]&&mySquare.x+mySquare.len-30<squares.x[i]+squares.w[i]
						...
						&&(mySquare.y+mySquare.len>squares.y[i]&&mySquare.y+mySquare.len<squares.y[i]+squares.h[i])){
						mySquare.y=squares.y[i]-mySquare.len;
						mySquare.toVSpeed=-totalSpeed*0.2;
						mySquare.toDownSpeed=0;
						count = 0;
					}
					if(mySquare.x+mySquare.len>squares.x[i]&&mySquare.x+mySquare.len<squares.x[i]+squares.w[i]
						&&mySquare.y+mySquare.len>squares.y[i]&&mySquare.y+mySquare.len<squares.y[i]+squares.h[i]
						){
						mySquare.x=squares.x[i]-mySquare.len;
					}			
